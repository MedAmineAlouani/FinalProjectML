...
code(
    ...
    "                    epochs=20, batch_size=32, val_frac=0.15):",
...
code(
    ...
    "                                seed=RANDOM_STATE, augment_fn=aug, epochs=20)",
...
code(
    "dl_task2_rows = []",
    "dl_task2_pool = {name: {'y_true': [], 'y_pred': []}",
    ...
    "    ]:",
    "        yhat, _ = train_dl_model(",
    "            factory, X_input[tr], y_labels[tr], X_input[te], y_labels[te],",
    "            seed=RANDOM_STATE + test_flange, augment_fn=aug, epochs=20)",
    "        acc = accuracy_score(y_labels[te], yhat)",
    "        dl_task2_rows.append({'Model': name, 'Test Flange': test_flange,",
    "                                'Accuracy (%)': acc * 100})",
    "        dl_task2_pool[name]['y_true'].extend(y_labels[te].tolist())",
    "        dl_task2_pool[name]['y_pred'].extend(yhat.tolist())",
    "        print(f'  {name:28s}  acc = {acc * 100:6.2f}%')",
    "",
    "dl_task2_df = pd.DataFrame(dl_task2_rows)",
    "dl_task2_summary = (dl_task2_df.groupby('Model')['Accuracy (%)']",
    "                     .mean().sort_values(ascending=False).reset_index()",
    "                     .rename(columns={'Accuracy (%)': 'Task 2 mean acc (%)'}))",
    "print('\\nDeep models — Task 2 mean hit-level accuracy:')",
    "display(dl_task2_summary)",
)

code(
    "dl_task2_preds = [(name, np.array(d['y_true']), np.array(d['y_pred']))",
    "                    for name, d in dl_task2_pool.items()]",
    "show_cm_grid(dl_task2_preds, 'Deep models — Task 2 confusion matrices (LOFO pooled)')",
)
...
code(
    ...
    "# ============================================================",
    "RUN_CRNN_TASKS = True  # set False to skip the inline CRNN retrain (~3 min on CPU)",
    "",
    "if RUN_CRNN_TASKS:",
