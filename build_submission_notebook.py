...
md(
    ...
    "**Flange-Invariant Logistic Regression** classifier with per-class",
    "isotonic calibration produces the final torque prediction via soft-vote",
    "across hits.",
    "",
    ...
    "13. Constrained competition prediction",
    "14. Conclusion",
...
md(
    ...
    "",
    "All three are evaluated under LOFO (train on 3 flanges, test on the 4th)",
    "with SpecAugment for regularization on the CNN/CRNN paths.",
    "",
    ...
    "Their test cells take a few minutes per fold on CPU.",
)
...
code(
    "# ============================================================",
    "# LOFO evaluation for the deep models",
    "# (set RUN_DL = True to actually train; left False for speed)",
    "# ============================================================",
    "RUN_DL = False  # flip to True to train the deep models live",
    "",
    "",
    ...
    "",
    "if RUN_DL:",
    "    cnn_summary  = lofo_dl_eval(make_cnn,  X_logmel,  'CNN (log-mel)',",
    "                                  augment_fn=spec_augment, epochs=35)",
    "    rnn_summary  = lofo_dl_eval(make_rnn,  X_mfccseq, 'RNN-GRU (MFCC seq)',",
    "                                  augment_fn=None,        epochs=35)",
    "    crnn_summary = lofo_dl_eval(make_crnn, X_logmel,  'CRNN (CNN+RNN, log-mel)',",
    "                                  augment_fn=spec_augment, epochs=35)",
    "else:",
    "    print('Deep learning evaluation skipped (RUN_DL=False).')",
    "    print('Previously measured LOFO file-level accuracies (Spring 2026):')",
    "    print('   CNN  : ~62 %')",
    "    print('   RNN  : ~58 %')",
    "    print('   CRNN : ~65 %')",
    "    print('All three are dominated by the tuned shallow models on this dataset.')",
)
...
md(
    ...
    "",
    "Diagnosis: in LOFO out-of-fold (OOF) predictions the LR has only ≈ 70%",
    "recall on the 25 ft-lbs class. Borderline true-25 hits average",
    "`P(25) ≈ 0.67` and lose to `P(50)` after argmax. We fit a per-class",
    "isotonic regressor on the LOFO OOF predictions — the calibrator never",
    "sees data the LR was trained on — and renormalize at inference.",
    "",
    "This correction lifts LOFO calibrated hit-level recall on class 25 from",
    "~70% to ~74% without harming the other two classes.",
)

code(
    "# Step 1: LOFO OOF raw LR probabilities",
    ...
    "",
    "# Step 2: fit one isotonic calibrator per class",
    "calibrators = {}",
    "for ci, c in enumerate(CLASSES_REF):",
    "    iso = IsotonicRegression(out_of_bounds='clip', y_min=0.0, y_max=1.0)",
    "    iso.fit(oof_proba[:, ci], (y_labels == c).astype(float))",
    "    calibrators[int(c)] = iso",
    "",
    "",
    "def calibrate_lr_proba(raw_proba_3col):",
    "    cal = np.column_stack([",
    "        calibrators[int(c)].predict(np.clip(raw_proba_3col[:, ci], 0.0, 1.0))",
    "        for ci, c in enumerate(CLASSES_REF)",
    "    ])",
    ...
    "oof_cal  = calibrate_lr_proba(oof_proba)",
    "cal_pred = CLASSES_REF[oof_cal.argmax(axis=1)]",
    "print('LOFO per-class recall: raw vs calibrated')",
    "for c in CLASSES_REF:",
    "    mask = y_labels == c",
    "    raw_r = (raw_pred[mask] == c).mean()",
    "    cal_r = (cal_pred[mask] == c).mean()",
    "    print(f'  class {c:3d}: raw {raw_r*100:5.2f}%   calibrated {cal_r*100:5.2f}%')",
    "print(f'  overall  : raw {(raw_pred == y_labels).mean()*100:5.2f}%   calibrated {(cal_pred == y_labels).mean()*100:5.2f}%')",
)
...
md(
    ...
    "5. Run the trained LR and apply isotonic calibration",
...
code(
    "# ============================================================",
    "# Predict per-hit + soft-vote per file",
    "# ============================================================",
    "if len(unlabeled_hits):",
    "    proba_lr_raw = champion_model.predict_proba(X_unlabeled_centered)",
    "    proba_lr_raw = proba_lr_raw[:, [list(champion_model.classes_).index(c)",
    "                                       for c in CLASSES_REF]]",
    "    proba_lr = calibrate_lr_proba(proba_lr_raw)",
    "",
    ...
    "",
    "    # Per-flange aggregate (the format the competition asks for)",
    ...
    "    })",
    "    print('\\nUnconstrained per-flange argmax:')",
    "    display(flange_pred)",
)
...
md(
    ...
    "# 13. Constrained competition prediction",
    "",
    "Per the professor's tip, the four test flanges satisfy a strong",
    "structural constraint:",
    "",
    "> Each torque value (0, 25, 50 ft-lbs) appears **at least once** across",
    "> the four flanges, and exactly one torque is repeated.",
    "",
    "So the answer is a permutation of one of three multisets:",
    "",
    "```",
    ...
    "```",
    "",
    "We enumerate every valid assignment, score each by the joint",
    "log-probability across the four flanges (using the soft-vote",
    "probabilities computed above), and pick the highest-scoring valid",
    "assignment. This guarantees the constraint is respected and",
    "tie-breaks ambiguities the model alone can't resolve.",
)
...
code(
    ...
    "# ============================================================",
    "if len(unlabeled_hits):",
    "    flange_proba_sorted = (per_hit",
    "                            .groupby('flange_id')[list(CLASSES_REF)]",
    "                            .mean()",
    "                            .sort_index())",
    "    print('Per-flange soft-vote probabilities (sorted):')",
    "    display(flange_proba_sorted.round(3))",
    "",
    "    valid_assignments = set()",
    "    for repeated in [0, 25, 50]:",
    "        multiset = [0, 25, 50, repeated]",
    "        for perm in _perms(multiset):",
    "            valid_assignments.add(perm)",
    "    print(f'\\n{len(valid_assignments)} valid assignments under the constraint.')",
    "",
    "    flange_ids_sorted = sorted(flange_proba_sorted.index.tolist())",
    "    best_assignment = None",
    "    best_log_score  = -np.inf",
    "    for assignment in valid_assignments:",
    "        log_score = 0.0",
    "        for fl, t in zip(flange_ids_sorted, assignment):",
    "            p = flange_proba_sorted.loc[fl, t]",
...
md(
    ...
    "          →  per-class isotonic calibration  →  soft-vote  →  argmax",
    "```",
    "",
    "### Final numbers",
    "",
    ...
    "| LOFO hit-level accuracy (calibrated)            | ~84% |",
...
md(
    ...
    "   file, pick a flange, and get the calibrated soft-vote prediction.",
    ...
    "",
    "Predictions in the web app are produced by exactly the same pipeline",
    "documented above (same feature extractor, same `keep_idx_final`, same",
    "`flange_means_final`, same trained `champion_model`, same isotonic",
    "calibrators).",
    "",
    "---",
    "",
