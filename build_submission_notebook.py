...
code(
    ...
    "",
    "### CRNN — Hit-level vs file-level breakdown",
    "",
    "| Test | Hit-level (pooled) | File-level (soft-vote) |",
    "|---|---|---|",
    "| Task 1 (70/30 dependent) | {task1_crnn:.2f}% | *(not applicable)* |",
    "| Task 2 LOFO              | {acc_crnn_t2_hit*100:.2f}%   | {lofo_crnn:.2f}%   |",
    "| Task 2 LOAO              | {acc_crnn_loao_hit*100:.2f}% | {loao_crnn:.2f}% |",
    "",
    "*Hit-level is per individual hit (~990 predictions); file-level",
    "averages the ~20 hit probabilities per file then argmaxes (48",
    "predictions). The ~1–3 point lift from hit→file is the value of",
    "soft-voting on this dataset.*",
    "",
